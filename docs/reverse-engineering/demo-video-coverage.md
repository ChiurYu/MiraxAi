...
| 演示功能 | 当前实现 | 代码位置 | 覆盖状态 |
| --- | --- | --- | --- |
| 对标视频导入 / 输入 | 左侧「项目素材」支持项目名、对标视频路径、声音样本路径输入 | `apps/desktop/src/App.vue` | 已覆盖，mock 输入 |
...
| 标题、封面、描述、话题 | 当前项目名和备注作为发布元信息；封面路径由 mock renderer 生成 | `apps/desktop/src/App.vue` | 部分覆盖，独立编辑面板待增强 |
...
在 `http://127.0.0.1:1420/` 中连续点击「运行下一步」8 次，预期结果：
