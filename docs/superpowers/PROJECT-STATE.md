...
阶段 5 P0：执行 `docs/superpowers/plans/2026-06-25-real-capability-foundation.md`。Task 1（GitHub Actions CI）与 Task 2（Provider 配置与真实调用边界）已完成，接下来执行 Task 3（梳理 sidecar / 本地依赖检测）。按 Task 3–6 顺序梳理 sidecar、媒体产物、工作台真实化与发布自动化的边界与顺序。不要提前实现真实 AI / 语音 / 数字人 / 视频 / 发布源码，除非对应 Task 已规划并验收。
