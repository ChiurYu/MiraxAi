...
阶段 5 P0 进行中：「真实能力接入前置基础设施与执行顺序」。Task 1（GitHub Actions CI）、Task 2（Provider 配置与真实调用边界）、Task 3（sidecar / 本地依赖检测）与 Task 4（真实媒体产物路径与错误状态）已完成：`@mirax/core` 新增 `ArtifactState` / `MediaArtifact` / 产物路径字段，`@mirax/media-pipeline` 新增 `artifactPaths.ts` 与 `MediaRendererError`，`mockRenderer` 路径改为 `outputRoot/<projectId>/<stage>/<fileName>` 且缺少前置产物时抛出诚实错误，`ffmpegCommands` 对空路径抛出诚实错误。Task 4 Review Fix 已清洗 `buildArtifactPath()` 的 `fileName`，去除 `.` / `..`、开头斜杠与反斜杠，保留正常内部层级。继续按 Task 5–6 梳理工作台真实化与发布自动化的边界与顺序。
...
阶段 5 P0：执行 `docs/superpowers/plans/2026-06-25-real-capability-foundation.md`。Task 1（GitHub Actions CI）、Task 2（Provider 配置与真实调用边界）、Task 3（sidecar / 本地依赖检测）与 Task 4（真实媒体产物路径与错误状态）已完成，接下来执行 Task 5（设计工作台各阶段从 mock 到真实能力的接入顺序）。按 Task 5–6 顺序梳理工作台真实化与发布自动化的边界与顺序。不要提前实现真实 AI / 语音 / 数字人 / 视频 / 发布源码，除非对应 Task 已规划并验收。
