...
当前实施分支：`codex/real-rewrite-provider-plan`。本分支用于下一阶段「Workbench rewrite 阶段接入 OpenAI-compatible LLM provider」的规划，不实现真实调用源码。
...
- Task 4（待派工）：设计 `ScriptRewritingStage` 的真实 / 未接入 / 失败状态（UI 诚实呈现，不伪造成功）。
