...
阶段 5 P6 compose 已完成本地实现、验收并进入 dogfood/readiness 阶段：真实 `avatarVideoPath + audioPath + subtitleText` → 真实 `finalVideoPath` / `coverPath` / `subtitlePath`；FFmpeg compose 命令已改为 avatar 视频 + speech 音频双输入，新增封面抽帧命令；desktop compose renderer 支持 real 路由，通过 Tauri `render_compose` command 调 Rust 层 FFmpeg；`VideoCompositionStage` 已接入 mock / real / not-connected / failed / success 状态。默认仍 mock；正常 UI 不再把未验证的 `sidecarConfig.ffmpegPath` 视为 real，真实失败不 fallback 到 mock，不伪造 `finalVideoPath` / `coverPath` / `subtitlePath`。安装版 dogfood 已通过一轮，当前 `main` 已 push。
...
阶段 5 P7 SQLite 本地存储主干已完成：`@mirax/local-store` 已实现 provider settings / app_settings、workbench_drafts、publish tasks、task_history 的 SQLite 优先、localStorage fallback 持久化。SQLite 迁移不再作为下一步安排。

阶段 5 P1 rewrite 文案改写能力已完成一轮优化：rewriteProviderConfigId 持久化；AI 服务页支持显式“设为文案改写 / 停止文案改写”；多 Provider 不再 fallback 到第一条；重新生成有运行状态反馈；改写目标 / 提示词模板 / 目标字数真实传入 rewriteScript；改写参数已持久化到 Workbench draft；已修复 active 但不可用时绿色“文案改写使用中”误导问题。
...
- 当前阶段 / first usable release dogfood & readiness：compose 真实渲染链路、SQLite 本地存储主干与 rewrite 文案改写能力均已完成；下一步聚焦 dogfood/readiness、真实发布链路或剩余 gap-list 最高优先级项。
- 已完成 compose 真实 FFmpeg renderer 规划：`docs/superpowers/plans/2026-06-29-real-compose-renderer.md`
...
阶段 4 P1 Stitch UI Vue 迁移已完成并合并。阶段 5 P0 已完成并合并（PR #3）。阶段 5 P1 rewrite 真实 LLM provider 已完成并合并（PR #4，CI passed）。阶段 5 P2 speech、P3 voice-clone、P4 transcribe、P5 avatar、P6 compose 已完成本地实现并进入 dogfood/readiness，P7 SQLite 本地存储主干已完成，rewrite 文案改写能力已完成一轮优化。当前自动调度入口为 **first usable release dogfood / readiness**，聚焦真实发布链路或剩余 gap-list 最高优先级项。

不再指向单一源码计划。已完成的 compose 计划见 `docs/superpowers/plans/2026-06-29-real-compose-renderer.md`；已完成的 SQLite 与改写能力计划当前为工作区未跟踪文档，默认不加入 Git，如需归档请由总控决定是否纳入版本控制。

## 最新可执行任务

阶段 4 P1 Stitch UI Vue 迁移全部 13 个 Task 已完成并合并，不再重复执行。阶段 5 P0 全部 6 个 Task 已完成并合并（PR #3），不再重复执行。阶段 5 P1 rewrite 全部 5 个 Task 已完成并合并（PR #4），不再重复执行。阶段 5 P6 compose Task 1–4 已完成，不再重复执行。SQLite 迁移已完成，不再安排。

当前已无单一源码计划作为自动调度入口。建议下一阶段从以下方向中按优先级选择一项进入：

1. **first usable release dogfood / readiness**：端到端跑通安装版，收集并修复阻塞性问题，补齐 readiness 检查清单（安装、升级、离线、错误恢复、关键路径监控）。
2. **真实发布链路**：将 `review -> publish` 从 mock 推进到真实平台发布，或至少完成 publisher 真实调用边界与失败恢复。
3. **剩余 gap-list 最高优先级项**：参考 `docs/reverse-engineering/legacy-ui-gap-list.md`，挑选对 first usable release 影响最大的未闭合 gap 进行验收或实现。

每次只执行一个 Task，完成并验证后同步更新本文件。不要修改 `docs/reverse-engineering/legacy-ui-gap-list.md` 的状态列；不要修改 `.codex/dispatch-state.json`。
...
first usable release dogfood / readiness 阶段。compose 真实 FFmpeg renderer 链路、SQLite 本地存储主干、rewrite 文案改写能力均已完成并进入 `main`；安装版 dogfood 已通过一轮，当前 `main` 已 push。

### 已完成

- 阶段 5 P6 compose 真实渲染链路已完成本地实现、验收并进入 dogfood/readiness。
- 阶段 5 P7 SQLite 本地存储主干已完成：provider settings / app_settings、workbench_drafts、publish tasks、task_history 实现 SQLite 优先、localStorage fallback。
- 阶段 5 P1 rewrite 文案改写能力已完成一轮优化：rewriteProviderConfigId 持久化；AI 服务页支持显式“设为文案改写 / 停止文案改写”；多 Provider 不再 fallback 到第一条；重新生成有运行状态反馈；改写目标 / 提示词模板 / 目标字数真实传入 rewriteScript；改写参数已持久化到 Workbench draft；已修复 active 但不可用时绿色“文案改写使用中”误导问题。
- 安装版 dogfood 已通过一轮，当前 `main` 已 push。

### 下一步建议

不再安排 SQLite 迁移。建议从以下方向中按优先级选择一项进入：

1. **first usable release dogfood / readiness**：端到端跑通安装版，收集并修复阻塞性问题，补齐 readiness 检查清单（安装、升级、离线、错误恢复、关键路径监控）。
2. **真实发布链路**：将 `review -> publish` 从 mock 推进到真实平台发布，完成 publisher 真实调用边界与失败恢复。
3. **剩余 gap-list 最高优先级项**：参考 `docs/reverse-engineering/legacy-ui-gap-list.md`，挑选对 first usable release 影响最大的未闭合 gap 进行验收或实现。
...
- `docs/superpowers/plans/2026-07-03-publish-task-history-sqlite-plan.md`
- `docs/superpowers/plans/2026-07-03-rewrite-provider-selection-plan.md`
- `docs/superpowers/plans/2026-07-03-workbench-draft-sqlite-plan.md`
- `docs/superpowers/specs/2026-07-03-publish-task-history-sqlite-design.md`
- `docs/superpowers/specs/2026-07-03-rewrite-provider-selection-design.md`
- `docs/superpowers/specs/2026-07-03-workbench-draft-sqlite-design.md`
- `manual-transcript-after-refresh.png`
- `manual-transcript-before-refresh.png`
- `mirax-actual-after-restart.png`
- `playwright-ai-services.png`
- `review-initial-before.png`
- `review-initial-state.png`
- `review-ready-state.png`

这些文件未纳入 Git。默认本次只更新 `PROJECT-STATE.md`，不将其加入提交。如需归档，建议由总控决定是否纳入版本控制：规划/设计文档可整理后迁入 `docs/superpowers/plans/` 或 `docs/superpowers/specs/`；截图如确需进入 Git，应按 `docs/reverse-engineering/assets/screenshots/` 规范存放并登记到 `assets-index.md` 与 `evidence-index.md`。
