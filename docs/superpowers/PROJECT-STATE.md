...
阶段 4 P1 Stitch UI Vue 迁移已完成并合并。阶段 5 P0 已完成并合并（PR #3）。下一阶段为 **rewrite 阶段接入 OpenAI-compatible LLM provider**。当前最新计划为：

**`docs/superpowers/plans/2026-06-26-real-rewrite-provider.md`**

该计划只进行规划，不实现真实 OpenAI 调用源码；后续按 Task 1–5 逐个派工执行。
...
- Task 5（待派工）：测试与验收计划（全链路自动化验收、手动 / 脚本验收、受保护文件检查）。
...
阶段 5 P0 已完成，PR #3 已合并到 `main`。下一阶段进入 **rewrite 阶段接入 OpenAI-compatible LLM provider** 的规划与实施：

1. 当前正在进行本阶段规划，见 `docs/superpowers/plans/2026-06-26-real-rewrite-provider.md`。
2. 规划完成后，按 Task 1–5 逐个派工执行。每个 Task 独立 review、独立验收，不可一次实现全部 Task。
3. Task 1 首先确认 provider-ai 层真实调用边界与错误类型；Task 2–4 在桌面端逐步接线；Task 5 做整体验收。
4. Task 1 需为 `OpenAiCompatibleProvider` 提供默认基于 `fetch` 的 runtime transport，使 rewrite real 模式能够发起真实 OpenAI-compatible 调用；单测通过注入 fake transport 避免联网。真实源码实现需等对应 Task 明确派工，不得提前跨 Task 修改。
