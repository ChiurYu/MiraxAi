...
阶段 5 P0 进行中：「真实能力接入前置基础设施与执行顺序」。Task 1（GitHub Actions CI）已落地：`.github/workflows/ci.yml` 已创建，覆盖 `push` 到 `main` 与所有 PR，自动执行 `pnpm test`、`pnpm typecheck`、`pnpm --filter @mirax/desktop build:web`。继续按 Task 2–6 梳理 Provider、sidecar、媒体产物、工作台真实化与发布自动化的边界与顺序。
...
- Task 2：梳理 Provider 配置与真实调用边界（API Key 不进日志 / snapshot / 任务 payload）。
- Task 3：梳理 sidecar / 本地依赖检测（FFmpeg、CosyVoice、HeyGem、Playwright）。
...
阶段 5 P0：执行 `docs/superpowers/plans/2026-06-25-real-capability-foundation.md`。Task 1（GitHub Actions CI）已完成，接下来执行 Task 2（梳理 Provider 配置与真实调用边界）。按 Task 2–6 顺序梳理 Provider、sidecar、媒体产物、工作台真实化与发布自动化的边界与顺序。不要提前实现真实 AI / 语音 / 数字人 / 视频 / 发布源码，除非对应 Task 已规划并验收。
