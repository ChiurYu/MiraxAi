...
下一阶段（阶段 5 P0）：「真实能力接入前置基础设施与执行顺序」。目标是在接入真实 AI / 语音 / 数字人 / 视频 / 发布之前，先补齐 CI、Provider 配置与调用边界、sidecar / 本地依赖检测、真实媒体产物路径与错误状态、工作台各阶段真实化顺序、发布自动化安全边界与失败恢复等前置规划，并落地最小可执行的 CI 与基础设施边界。
...
- Task 1：补 GitHub Actions CI（`pnpm test` / `pnpm typecheck` / `desktop build:web`）。
...
阶段 5 P0：执行 `docs/superpowers/plans/2026-06-25-real-capability-foundation.md`。先落地 Task 1（GitHub Actions CI），再按 Task 2–6 梳理 Provider、sidecar、媒体产物、工作台真实化与发布自动化的边界与顺序。不要提前实现真实 AI / 语音 / 数字人 / 视频 / 发布源码，除非对应 Task 已规划并验收。
