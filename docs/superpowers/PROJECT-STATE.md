...
当前实施分支：`codex/real-rewrite-provider-plan`。本分支用于「Workbench rewrite 阶段接入 OpenAI-compatible LLM provider」的规划与逐步实施。Task 1（provider-ai 真实调用边界与错误类型）、Task 2（rewrite executor 路由与 provider 选择工厂）、Task 3（API Key / baseUrl / model 配置读取边界）与 Task 4（`ScriptRewritingStage` 四态 UI）已全部完成并通过自动验证；Task 5（整体验收）待派工。真实 OpenAI 调用源码已在 Task 1 中实现默认 fetch transport，后续 Task 5 只做整体验收与缺口补测，不再新增真实能力。
...
- **下一阶段 / rewrite 真实 LLM provider 规划：`docs/superpowers/plans/2026-06-26-real-rewrite-provider.md`**
...
阶段 4 P1 Stitch UI Vue 迁移已完成并合并。阶段 5 P0 已完成并合并（PR #3）。当前阶段 **rewrite 阶段接入 OpenAI-compatible LLM provider** 已完成 Task 1–5，等待总控 push / PR。

**`docs/superpowers/plans/2026-06-26-real-rewrite-provider.md`**

该计划已完成 provider-ai 真实调用边界、桌面端 rewrite 路由、凭证读取边界、`ScriptRewritingStage` 四态 UI 与整体验收。

## 最新可执行任务

阶段 4 P1 Stitch UI Vue 迁移全部 13 个 Task 已完成并合并，不再重复执行。阶段 5 P0 全部 6 个 Task 已完成并合并（PR #3），不再重复执行。

下一阶段最新可执行任务见 `docs/superpowers/plans/2026-06-26-real-rewrite-provider.md`（规划文档，未实现源码）：

- Task 1（已完成，含 review fix）：确认 provider-ai 真实调用边界与错误类型（`@mirax/provider-ai` 内闭环）。
- Task 2（已完成）：设计 rewrite executor 如何从 mock 切到 real（桌面端路由与 provider 选择工厂）。Review Fix 已修复 provider 选择顺序：仅当 provider 为 openai/custom 时才被视为合法 rewrite provider。
- Task 3（已完成）：设计 API Key / baseUrl / model 配置读取边界（安全读取、snapshot 无 key、日志无凭证）。Review Fix 已收紧：rewrite real 路径在构造 provider 前使用 `sanitizeBaseUrlForStorage` 清洗 baseUrl，避免 token/query/hash 进入真实 endpoint。
- Task 4（已完成）：设计 `ScriptRewritingStage` 的真实 / 未接入 / 失败状态（UI 诚实呈现，不伪造成功）。实现：新增 `mode` / `errorMessage` props 与 `isMock / isReal / isNotConnected / hasError / canRun / modeLabel / resultPlaceholder` 计算属性；四态横幅分别提示「Mock 结果」、未连接禁用生成、真实调用信息、失败错误摘要；版本记录收敛为 `shouldRecordVersion(prev, next, script, mode)`，仅 mock / real 成功时记录，`not-connected` 不记录；敏感字段不进入 UI；`App.vue` 传入 `rewriteMode` 与 `rewriteErrorMessage`；新增 `ScriptRewritingStage.test.ts`（12 tests）与 `scriptRewritingStage.utils.ts`。
- Task 5（已完成）：测试与验收计划（全链路自动化验收、手动 / 脚本验收、受保护文件检查）。复验 provider / desktop / UI 相关 78 tests、workbench 21 tests、全仓 187 tests、`pnpm typecheck`、`pnpm --filter @mirax/desktop build:web`、`git diff --check` 与受保护文件 diff，均通过。
...
阶段 5 P0 已完成，PR #3 已合并到 `main`。**rewrite 阶段接入 OpenAI-compatible LLM provider** 的 Task 1–5 已完成并通过本地验证：

1. 当前计划与验收记录见 `docs/superpowers/plans/2026-06-26-real-rewrite-provider.md`。
2. 最新提交序列已覆盖 Task 1–5；下一步是 push 当前分支并创建 PR。
3. 验证基线：`pnpm test`、`pnpm typecheck`、`pnpm --filter @mirax/desktop build:web` 均通过；测试不依赖真实 LLM 网络。
4. 未实现 keychain / OS 安全存储；apiKey 仍为当前会话内存字段，真实持久化凭证留待后续专项。
