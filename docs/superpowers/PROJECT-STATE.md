...
阶段 5 P0 进行中：「真实能力接入前置基础设施与执行顺序」。Task 1（GitHub Actions CI）、Task 2（Provider 配置与真实调用边界）、Task 3（sidecar / 本地依赖检测）、Task 4（真实媒体产物路径与错误状态）、Task 5（工作台各阶段从 mock 到真实能力的接入顺序）与 Task 6（发布自动化的账号、凭证、安全边界与失败恢复）已完成：`@mirax/provider-publish` 扩展 `PublishTaskStatus`、`PublishTask` 失败恢复字段与 `PublishHandoffResult` per-platform 子结果；`mockPublisher` 返回更真实失败场景；`local-store` schema 增加 `credential_ref` 与任务错误/重试字段；`AccountManagementView` 调整为诚实授权 handoff UI；`publishTaskStore` 防御性过滤凭证并归一化旧数据；`usePublishPreparation` 根据 `platformResults` 创建 `submitted` / `failed` / `retryable` 任务；`docs/product-architecture/publish-automation-security-design.md` 已创建。继续按阶段 5 P0 后续规划推进。
...
- Task 4：设计真实媒体产物路径与错误状态（不伪造文件、时长、分辨率）——已完成，`@mirax/core` 与 `@mirax/media-pipeline` 产物/错误边界已落地。
- Task 5：设计工作台各阶段从 mock 到真实能力的接入顺序——已完成，`docs/product-architecture/workbench-real-capability-rollout.md` 已落地，`@mirax/core` 阶段依赖与 `useWorkflowRuntime` 模式选择已补充；未改动 stage 组件 UI，能力模式展示属于后续 polish。
- Task 6：设计发布自动化的账号、凭证、安全边界与失败恢复——已完成，`docs/product-architecture/publish-automation-security-design.md` 已落地，类型、schema、mock publisher、账号 UI、任务 store 与 `usePublishPreparation` 已同步。
...
阶段 5 P0：执行 `docs/superpowers/plans/2026-06-25-real-capability-foundation.md`。Task 1（GitHub Actions CI）、Task 2（Provider 配置与真实调用边界）、Task 3（sidecar / 本地依赖检测）、Task 4（真实媒体产物路径与错误状态）、Task 5（工作台各阶段真实化顺序）与 Task 6（发布自动化安全边界与失败恢复）已完成。下一步按阶段 5 P0 后续规划推进；若计划内无更多 Task，则进入下一阶段规划。
