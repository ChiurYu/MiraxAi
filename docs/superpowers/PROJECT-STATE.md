...
阶段 5 P0 进行中：「真实能力接入前置基础设施与执行顺序」。Task 1（GitHub Actions CI）、Task 2（Provider 配置与真实调用边界）、Task 3（sidecar / 本地依赖检测）、Task 4（真实媒体产物路径与错误状态）与 Task 5（工作台各阶段从 mock 到真实能力的接入顺序）已完成：`@mirax/core` 新增 `WorkflowStageRuntimeMode`、阶段依赖 `STAGE_PREREQUISITES`、推荐真实化顺序与默认 mock 模式；`useWorkflowRuntime` 支持按阶段配置 `mock/real/not-connected` 并提供 `getStageMode`。Task 5 未改动任何 Workbench stage 组件 UI；模式展示等 UI 优化属于后续 UI polish，不在本 Task 范围内。继续按 Task 6 梳理发布自动化的边界与顺序。
...
阶段 5 P0：执行 `docs/superpowers/plans/2026-06-25-real-capability-foundation.md`。Task 1（GitHub Actions CI）、Task 2（Provider 配置与真实调用边界）、Task 3（sidecar / 本地依赖检测）、Task 4（真实媒体产物路径与错误状态）与 Task 5（工作台各阶段真实化顺序）已完成，接下来执行 Task 6（发布自动化的账号、凭证、安全边界与失败恢复）。按 Task 6 顺序梳理发布自动化边界。不要提前实现真实 AI / 语音 / 数字人 / 视频 / 发布源码，除非对应 Task 已规划并验收。
