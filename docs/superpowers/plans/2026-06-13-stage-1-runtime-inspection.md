...
**目标：** 在可打开但可能受限的旧版「轻语 IP 智能体」中，按页面和流程完整记录运行态证据、页面结构、受限动作和初版功能卡。
...
执行顺序：先完成 Task 1 的巡检准备，再按 P0 → P1 → P2 页面优先级记录页面卡、功能卡和运行障碍。
...
| EV-RUNTIME-100 到 EV-RUNTIME-199 | P1 管理页面。 |
...
如果使用完整录屏，不提交录屏文件，只在 `assets-index.md` 记录本机路径或外部位置。
...
rg -n "EV-RUNTIME-001|EV-RUNTIME-010|EV-RUNTIME-100|EV-RUNTIME-200" docs/reverse-engineering/evidence-index.md
...
从旧版 App 导航进入设置页。记录模型配置、密钥、Base URL、服务状态、本地依赖、账号或授权相关入口。
...
从首页、任务中心或账号管理页进入发布相关流程。记录平台、账号、标题、描述、封面、话题、发布时间、草稿或直接发布等控件。
...
记录声音列表、上传入口、克隆入口、试听、状态、删除或编辑动作，写入 `PAGE-VOICE-MANAGEMENT.md`。

- [ ] **Step 2：巡检形象管理**

记录数字人形象列表、上传入口、训练或生成入口、封面、状态、删除或编辑动作，写入 `PAGE-AVATAR-MANAGEMENT.md`。

- [ ] **Step 3：巡检素材管理**

记录素材分类、上传入口、搜索、标签、预览、删除或编辑动作，写入 `PAGE-MATERIALS.md`。

- [ ] **Step 4：巡检任务中心**

记录任务列表、状态、进度、失败原因、重试、查看结果等动作，写入 `PAGE-TASK-CENTER.md`。

- [ ] **Step 5：巡检账号管理**

记录平台账号列表、登录状态、添加账号、删除账号、浏览器 profile 或授权提示，写入 `PAGE-ACCOUNT-MANAGEMENT.md`。
