...
import { loadPublishTasks, savePublishTasks } from "../../features/task-center/publishTaskStore.js";
...
const statusMap: Record<PublishTask["status"], { label: string; icon: unknown }> = {
...
<template>
  <section v-if="tasks.length > 0" class="task-preview">
    <h3>发布任务队列</h3>
    <ul class="task-list">
      <li v-for="task in tasks.slice(0, limit)" :key="task.id" class="task-item" :data-status="task.status">
        <div class="task-main">
